...
<div class="page-title">
    <div class="title_left">
        <h3><?php echo date("F j, Y", strtotime($this->uri->segment(3)))?> Record</h3>
    </div>
</div>
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    function fill_table(){
        ...
        $.post('<?php echo base_url(); ?>' + 'index.php/record/daily_record_fetch/'+ date +'?id='+id, {POST: true}, function(data, status){
                var actions = null;
                ...
                if(!data.error){
                    ...
                    data.forEach(function(item) { //insert rows
                        ...
                        if(item.status == 1)
                        {
                            var status = '<span style = "color: green">Approved</span>';
                        }
                        else
                        {
                            var status = '<span style = "color: red">For Approval</span>';
                        }

                        actions =
                            '<a class="btn btn-info my-ttip" href="<?php echo base_url(); ?>record/profile_info/' + item.id + '" data-index="' + item.id + '" data-toggle="tooltip" data-title="See Details" title=""><span class="glyphicon glyphicon-eye-open"></span></a>'
                            
                            ;

                        enumeratorDataTable.row.add([item.firstname+' '+item.middlename+' '+item.lastname ,item.email, status,actions]);
